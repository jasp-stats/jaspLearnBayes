...
## two players
{
  ...
  test_that("Game of Chance two-player plot matches", {
    ...
    jaspTools::expect_equal_plots(testPlot, "two-player CIPlot", dir = "LSgameofchance")
  })
  
  test_that("Game of Chance summaryTable two-player results match", {
    table <- results[["results"]][["summaryTable"]][["data"]]
    jaspTools::expect_equal_tables(table,
                                   list(0.037037037037037, 0.333333333333333, 
                                        0.032, 1, 1, 0.962962962962963,
...
## three players
{
  ...
  set.seed(1)
  dataset <- NULL
  results <- runAnalysis("LSgameofchance", dataset, options)
  
  
  test_that("Probability of Player 1 Winning plot matches", {
    plotName <- results[["results"]][["CIPlot"]][["data"]]
    testPlot <- results[["state"]][["figures"]][[plotName]][["obj"]]
    jaspTools::expect_equal_plots(testPlot, "probability-of-player-1-winning", dir="LSgameofchance")
  })
  
  test_that("Summary Table results match", {
    table <- results[["results"]][["summaryTable"]][["data"]]
    jaspTools::expect_equal_tables(table,
                                   list(0.3125, 0.5, 0.258, 1, 1, 0.6875, 0.5, 0.742, 2, 2))
  })
}
