...
.readyBinomialLS       <- function(options) {
  ...
  ready <- c("data" = readyData, "models" = readyPriors)
...
.estimateBinomialLS         <- function(data, prior) {
  ...
  if (prior[["type"]] == "spike") {
    ...
    textLower <- .formatFractionInput(prior[["priorTruncationLowerInp"]], lower)
    textUpper <- .formatFractionInput(prior[["priorTruncationUpperInp"]], upper)
  ...
  for (i in 1:length(models)) {
    ...
    if (data$nSuccesses + data$nFailures > 0) {
      ...
      if (tempPrior[["type"]] == "spike") {
        ...
        logLik[i]   <- stats::dbinom(data$nSuccesses, data$nSuccesses + data$nFailures, tempPrior[["spikePoint"]], log = TRUE)
...
.predictBinomialValuesLS    <- function(data, prior, n) {
  ...
  if (prior[["type"]] == "spike") {
    ...
    y <- .dbbinomLS(x, n, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]])
  }
...
.marginalSupportBinomialLS  <- function(data, models, postDensity, postSpikes, BF) {

  # posterior spikes and density are already computed, we just need to get models
  priorSpikes   <- list()
  densityI      <- 0
  priorDensity  <- NULL
  tempResults   <- .testBinomialLS(data, models)
  for (i in 1:length(models)) {
    if (models[[i]]$type == "spike") {
      priorSpikes <- c(
        priorSpikes,
        list(data.frame(y = models[[i]]$priorWeight, x = models[[i]]$spikePoint, g = "__marginal"))
      )
    } else if (models[[i]]$type == "beta") {
      dfLinesPP   <- .dataLinesBinomialLS(data, models[[i]])
      dfLinesPP   <- dfLinesPP[dfLinesPP$g == "Prior",]
      dfLinesPP$y <- exp(log(dfLinesPP$y)+log(tempResults[i, "prior"]))
      dfLinesPP$g <- models[[i]]$name
...
.dataHPDBinomialLS          <- function(data, prior, coverage, n = NULL, type = c("parameter", "prediction")) {
  ...
  if (type == "parameter") {
    ...
    if (prior[["type"]] == "spike")
      ...
      x <- .betabinomialHDILS(n, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]], coverage)
...
.dataCentralBinomialLS      <- function(data, prior, coverage, n = NULL, type = c("parameter", "prediction")) {
  ...
  if (type == "parameter") {
    ...
    if (prior[["type"]] == "spike")
      ...
      x <- c(
        .qbbinomLS((1 - coverage)/2 + 1e-5, n, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]]),
        .qbbinomLS(1 - (1 - coverage)/2,     n , prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]])
      )
...
.dataCustomBinomialLS       <- function(data, prior, lCI, uCI, n = NULL, type = c("parameter", "prediction")) {
  ...
  if (type == "parameter") {
    ...
    if (prior[["type"]] == "spike")
      ...
      coverage <- sum(sapply(lCI:uCI, function(s)
        .dbbinomLS(s, n, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]])))
...
.estimateDataPointBinomial  <- function(data, prior, N, type = c("parameter", "prediction"), estimate = c("mean", "median", "mode"), prop = FALSE) {
  ...
  if (type == "parameter") {
    ...
    if (prior[["type"]] == "spike") {
      ...
      if (estimate == "mode" && prior[["betaPriorAlpha"]] + data$nSuccesses == 1 && prior[["betaPriorBeta"]] + data$nFailures == 1) {
        x <- NA
        y <- NA
    ...
    if (prior[["type"]] == "spike") {
      if (.is.wholenumber((N + 1)*prior[["spikePoint"]]) && estimate == "mode")
        x <- c( (N + 1)*prior[["spikePoint"]], (N + 1)*prior[["spikePoint"]]-1) / d
      else
      ...
      if (estimate == "mode" && !is.numeric(x)) {
        if (prior[["betaPriorAlpha"]] + data$nSuccesses == 1 && prior[["betaPriorBeta"]] + data$nFailures == 1)
          x <- c(NA)
        else if (prior[["betaPriorAlpha"]] + data$nSuccesses < 1 && prior[["betaPriorBeta"]] + data$nFailures < 1 &&
                 prior[["betaPriorAlpha"]] + data$nSuccesses == prior[["betaPriorBeta"]] + data$nFailures)
          x <- c(0, N) / d
        else {
          x <- .dbbinomLS(0:N, N, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]])
          x <- c(0:N)[x == max(x)] / d
        }
      }
      y <- .dbbinomLS(x * d, N, prior[["betaPriorAlpha"]] + data$nSuccesses, prior[["betaPriorBeta"]] + data$nFailures, prior[["priorTruncationLower"]], prior[["priorTruncationUpper"]])
    }
...
.dataPointMarginalBinomial  <- function(data, options, allLines, allSpikes, N, type = c("parameter", "prediction"), type2 = c("Prior", "Posterior"), estimate = c("mean", "median", "mode"), prop = FALSE) {
  ...
  if (estimate == "median") {
    ...
    if (type == "parameter") {
      ...
      tempEstimates <- sapply(options[["models"]],function(prior).estimateBinomialLS(data, prior), simplify = F)
      ...
      tempPredictions <- sapply(options[["models"]],function(prior).predictBinomialLS(data, prior, options, prop), simplify = F)
